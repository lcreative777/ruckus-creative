...
### Prerequisites to verify before building this

- The `ruckuscreative.com` zone is on account `c0780521925c950ef323a873c907c291`.
- Email Routing is enabled on that zone.
- A destination address is **verified** in Email Routing. `send_email` will only deliver to a verified destination — this is a hard platform constraint, and the binding fails at runtime if it isn't met.

These are checked in Phase 1, before any form code is written, because a failure here changes the approach.
...
- **Structured data**, newly added — the current site has none: `Organization` and `LocalBusiness` (real address, 27525 Puerta Real Suite 300-173, Mission Viejo CA 92691, and phone 714-514-1482), `BreadcrumbList`, `Article` on knowledge posts, `CreativeWork` on portfolio items.
